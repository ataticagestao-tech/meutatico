...
export default function TermosPage() {
    ...
    async function init() {
      try {
        const [tplRes, clientsRes, histRes] = await Promise.all([
          api.get("/templates?category=termo"),
          api.get("/clients"),
          api.get("/templates/generated/list?category=termo"),
        ]);
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const histRes = await api.get("/templates/generated/list?category=termo");
      setGeneratedDocs(Array.isArray(histRes.data) ? histRes.data : []);
    } catch {
      // Handle error
    } finally {
        ...
        <div className="bg-background-primary border border-border rounded-xl">
          ...
          {generatedDocs.length === 0 ? (
            <div className="p-8 text-center">
              ...
              <p className="text-sm text-foreground-tertiary">Nenhum termo gerado</p>
            </div>
          ) : (
                ...
                <div key={doc.id} className="flex items-center justify-between p-4">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-foreground-tertiary mt-0.5">
                      {doc.template_name} — v{doc.version} —{" "}
        ...
        <>
          ...
          <div className="flex items-center gap-2 mb-6">
            {["template", "client", "preview"].map((s, i) => {
              const labels = ["1. Tipo de Termo", "2. Cliente", "3. Preview"];
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {templates.length === 0 ? (
                <div className="col-span-full bg-background-primary border border-border rounded-xl p-8 text-center">
                  <ShieldCheck size={32} className="mx-auto mb-2 text-foreground-tertiary opacity-50" />
                  <p className="text-sm text-foreground-tertiary">
                    Nenhum template de termo disponível
                  </p>
                </div>
              ) : (
                templates.map((tpl) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className="p-2 bg-emerald-100 dark:bg-emerald-900/20 rounded-lg shrink-0">
                ...
                <div className="flex items-center gap-2">
                  {generatedResult ? (
                    ...
                    >
                      ...
                      {generating ? "Gerando..." : "Gerar Termo"}
                    </button>
                  )}
                </div>
              </div>
              ...
              {previewLoading ? (
                ...
                <div className="bg-white border border-border rounded-xl overflow-hidden shadow-sm">
                  <div className="p-2 bg-background-secondary border-b border-border flex items-center gap-2">
                    ...
                    <span className="text-xs text-foreground-tertiary">Preview do termo</span>
                  </div>
                  <div
                    className="p-6"
